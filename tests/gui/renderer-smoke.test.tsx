...
test('renders the redesigned Home view with sidebar navigation', { timeout: 15_000 }, async () => {
  const harness = createBridgeHarness();
  render(<App desktop={harness.bridge} />);

  // Sidebar with 5 nav buttons
  ...
  expect(await screen.findByRole('button', { name: 'Open coverage' })).toBeInTheDocument();
  expect(await screen.findByRole('button', { name: 'Live-site viewer' })).toBeInTheDocument();
});

test('navigates between shell tabs without crashing', { timeout: 15_000 }, async () => {
  const harness = createBridgeHarness();
  render(<App desktop={harness.bridge} />);

  // Coverage tab → Coverage Explorer rendered inline
  ...
  // Browse tab → inline viewer heading
  fireEvent.click(await screen.findByRole('tab', { name: 'Browse' }));
  expect((await screen.findAllByRole('heading', { name: 'Browse' })).length).toBeGreaterThanOrEqual(1);

  // Data tab → Data Explorer heading
  ...
  // Settings tab → Workspace/login cards
  fireEvent.click(await screen.findByRole('tab', { name: 'Settings' }));
  expect(await screen.findByRole('heading', { name: 'Workspace' })).toBeInTheDocument();
  expect(await screen.findByRole('heading', { name: 'PBInfo login' })).toBeInTheDocument();

  // Back to Home
  fireEvent.click(await screen.findByRole('tab', { name: 'Home' }));
  expect(await screen.findByText('Archive health')).toBeInTheDocument();
  void harness;
});
