  ...
  return (
    <section className="panel data-panel panel-workspace">
      ...
      <div className="data-explorer-grid">
        ...
        <DataRecordDetail
          detail={detail}
          liveMirrorRecordUrl={liveMirrorRecordUrl}
...
function DataRecordList(props: {
  ...
  const { archiveQuery, listing, selectedRecordId, onArchiveQueryChange, onSelectRecord } = props;
...
function DataRecordDetail(props: {
  detail: GuiArchiveRecordDetail | null;
  liveMirrorRecordUrl?: string;
  onOpenPath: (path: string) => Promise<unknown>;
  onOpenExternal: (url: string) => Promise<unknown>;
}) {
  const { detail, liveMirrorRecordUrl, onOpenPath, onOpenExternal } = props;
  if (!detail) {
    return (
